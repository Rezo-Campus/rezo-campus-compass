...
import {
  ...
  Phone, StickyNote, LogIn, ChevronLeft,
} from "lucide-react";
...
function BookingPage() {
  ...
  const [googleUser, setGoogleUser]     = useState<{ id: string; name: string; email: string } | null>(null);

  /* ── Vérification auth Google au démarrage ── */
  useEffect(() => {
  ...
  /* ── Queries (activées seulement si connecté) ── */
  ...
  /* ── Mutation réservation ── */
  const book = useMutation({
    mutationFn: async () => {
      if (!selectedSlot || !googleUser) throw new Error("Données manquantes.");

      const { error: bookErr } = await db.from("interview_bookings").insert({
        slot_id:         selectedSlot.id,
        session_id:      sessionId,
        applicant_name:  googleUser.name,
        applicant_email: googleUser.email,
    ...
    onSuccess: () => { setStep("done"); refetchSlots(); },
    onError: (e: Error) => {
      ...
      <PageWrapper>
        <div className="mx-auto max-w-sm py-8">
          <div className="rounded-2xl border border-border bg-card p-8 text-center shadow-sm">
            ...
            <h2 className="mb-2 text-xl font-bold">Connexion requise</h2>
            <p className="mb-6 text-sm text-muted-foreground">
              Connectez-vous avec votre compte Google pour consulter les créneaux disponibles et réserver votre entretien.
            </p>
            <Button
              size="lg"
  ...
  /* ══════════════════════════════════
     CHARGEMENT SESSION / CRÉNEAUX
  ══════════════════════════════════ */
  if (loadingSession || loadingSlots) {
    return (
      <PageWrapper>
        <div className="grid min-h-[50vh] place-items-center">
          <Loader2 className="size-8 animate-spin text-primary" />
    ...
    return (
      <PageWrapper session={session}>
        <div className="mx-auto max-w-lg rounded-2xl border border-green-200 bg-green-50 p-8 text-center">
          ...
          <h2 className="mb-2 text-xl font-bold text-green-800">Entretien confirmé !</h2>
          <p className="mb-1 font-medium text-green-700">Bonjour {googleUser.name},</p>
          ...
          <p className="mt-6 text-xs text-muted-foreground">
            Prenez note de ces informations. Un rappel vous sera éventuellement envoyé par e-mail à <strong>{googleUser.email}</strong>.
          </p>
        </div>
      </PageWrapper>
    );
    ...
    return (
      <PageWrapper session={session}>
        <div className="mx-auto max-w-lg">
          ...
          {/* Identité Google (non modifiable) */}
          <div className="mb-5 rounded-xl border border-green-200 bg-green-50 px-5 py-4">
            <div className="flex items-center gap-3">
              <CheckCircle2 className="size-5 shrink-0 text-green-600" />
              <div>
                <div className="font-semibold text-sm text-green-800">{googleUser.name}</div>
          ...
          <div className="space-y-4">
            <div className="grid gap-2">
              <Label className="flex items-center gap-1.5">
                <Phone className="size-3.5" /> Téléphone <span className="text-muted-foreground">(facultatif)</span>
              </Label>
              <Input
                type="tel"
                value={phone}
            ...
            <div className="grid gap-2">
              <Label className="flex items-center gap-1.5">
                <StickyNote className="size-3.5" /> Message <span className="text-muted-foreground">(facultatif)</span>
              </Label>
              <Textarea
                rows={3}
                value={notes}
  ...
  const slotsByDay = slots.reduce<Record<string, Slot[]>>((acc, sl) => {
    const key = new Date(sl.starts_at).toLocaleDateString("fr-FR", {
  ...
  return (
    <PageWrapper session={session}>
      ...
      <div className="mx-auto mb-6 flex max-w-2xl items-center justify-between rounded-xl border border-green-200 bg-green-50 px-4 py-2.5">
        ...
        <button
          ...
          onClick={() => supabase.auth.signOut().then(() => setGoogleUser(null))}
        >
          Changer de compte
        </button>
      </div>
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mx-auto flex max-w-3xl items-center gap-4">
          ...
          {session && (
            <div className="min-w-0 flex-1 border-l border-border pl-4">
              ...
              <div className="truncate text-xs text-muted-foreground">Poste : {session.position}</div>
            </div>
          )}
        </div>
      </header>
      ...
      <footer className="border-t border-border py-6 text-center text-xs text-muted-foreground space-y-1">
        <p>
          ...
          <a href="https://rezocampus.fr/" target="_blank" rel="noopener noreferrer"
...
function Detail({ icon, label, value, isLink }: {
